...
class TicketsServiceTest {
	...
	void validateTicketStateDone_shouldThrowTicketStateNotDoneException_whenTicketStatusIsNotDone() {
		...
		TicketStateNotDoneException exception = assertThrows(TicketStateNotDoneException.class, () -> {
			ticketsService.validateTicketStateDone(TicketUtil.TICKET_ID);
		});
		// then
		assertInstanceOf(TicketStateNotDoneException.class, exception);
		assertEquals(String.format("Status for ticket is not DONE! ticketId:%s", TicketUtil.TICKET_ID),
	...
	void validateTicketOwnerBelongUser_shouldThrow() {
		...
		TicketOwnerNotBelongsToUserException exception = assertThrows(TicketOwnerNotBelongsToUserException.class,
				() -> {
					ticketsService.validateTicketOwnerBelongUser(TicketUtil.TICKET_ID, TicketUtil.USER_ID);
				});
		// then
		assertInstanceOf(TicketOwnerNotBelongsToUserException.class, exception);
		assertEquals(String.format("Ticket owner not belongs to user for ticket! ticketId=%s, userId:%s",
				TicketUtil.TICKET_ID, TicketUtil.USER_ID), exception.getMessage());
		verify(ticketMapper).mapTicketInTicketDto(any(Ticket.class));
		verify(ticketsRepository).findById(anyLong());
	}
}
