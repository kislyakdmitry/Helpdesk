...
public class TicketsService {
	...
	public void updateTicket(TicketUpdateDto updateDto, Long id) {
		Ticket ticket = ticketsRepository.findById(id).orElseThrow(() -> new TicketNotFoundException(id));
	...
	public Page<TicketDto> getMyTickets(User user, Pageable pageable, FilterParamsDto filterParams) {
		...
		return ticketsRepository.findAll(where(hasOwnerId(user.getId()).and(hasUrgency(
				filterParams.getUrgencies().isEmpty() ? List.of(Urgency.values()) : filterParams.getUrgencies())))
						.and(hasState(filterParams.getStates().isEmpty()
						...
						.and(filterParams.getDesiredDate() == null
								? null
								: hasDesiredDate(filterParams.getDesiredDate())),
				pageable).map(ticketMapper::mapTicketInTicketDto);
	}

	public Page<TicketDto> getTicketsForManager(User user, Pageable pageable, FilterParamsDto filterParams) {
		return ticketsRepository.findTicketsForManager(user.getId(), TicketServiceUtil.statesOfManagerApprover,
				filterParams.getId(), filterParams.getName(), filterParams.getDesiredDate(),
	...
	public Page<TicketDto> getTicketsForEngineer(User user, Pageable pageable, FilterParamsDto filterParams) {
		return ticketsRepository.findTicketsForEngineer(user.getId(), TicketServiceUtil.statesOfEngineerAssignee,
				filterParams.getId(), filterParams.getName(), filterParams.getDesiredDate(),
				filterParams.getStates().isEmpty() ? Arrays.stream(State.values()).toList() : filterParams.getStates(),
				filterParams.getUrgencies().isEmpty()
						? Arrays.stream(Urgency.values()).toList()
	...
	public FilterParamsDto getFilterParamsDto(Long id, String name, String desiredDate, Urgency[] urgencies,
			State[] states) {
