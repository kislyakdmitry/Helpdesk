...
public class FeedbackService {
	...
	public FeedbackDto getFeedbackById(Long feedbackId) {
		return feedbackRepository.findById(feedbackId).map(feedbackMapper::mapFeedbackToFeedbackDto)
				.orElseThrow(() -> new FeedbackNotFoundException(feedbackId));
	}

	public FeedbackDto getFeedbackByTicketId(Long ticketId) {
		return feedbackRepository.findFeedbackByTicketId(ticketId).map(feedbackMapper::mapFeedbackToFeedbackDto)
